...
1.  **Orchestrate Architecture**: Apply the [Architecture Skill](.agents/skills/flutter-apply-architecture-best-practices/SKILL.md) to define feature boundaries.
2.  **Define Contracts**: Establish domain interfaces and models.
3.  **Implement Data Layer**: Build Retrofit clients and Repository implementations using the [Networking Skill](.agents/skills/flutter-use-dio-retrofit/SKILL.md).
4.  **Wire Dependencies**: Register everything in GetIt using the [DI Skill](.agents/skills/flutter-use-get-it/SKILL.md).
5.  **Build Business Logic**: Implement BLoCs following the [BLoC Skill](.agents/skills/flutter-use-bloc/SKILL.md) conventions.
6.  **Handle Permissions**: Implement runtime permission boundaries using the [Permission Skill](.agents/skills/flutter-use-permission-handler/SKILL.md).
7.  **Localize**: Add strings using the [Localization Skill](.agents/skills/flutter-setup-localization/SKILL.md).
8.  **Construct UI**: Build the screen and components using the [UI Components Skill](.agents/skills/flutter-build-ui-components/SKILL.md).
9.  **Dynamic Widget State**: Build interactive widgets using the [WidgetState Skill](.agents/skills/flutter-add-widget-dynamic-state-widget-state/SKILL.md).
10. **Verify**: Create [Widget Previews](.agents/skills/flutter-add-widget-preview/SKILL.md) and [Tests](.agents/skills/flutter-add-widget-test/SKILL.md).
11. **Error Handling**: [Error Handling Skill](.agents/skills/flutter-handle-errors/SKILL.md).
12. **Final Integration**: Run `launch_app` via MCP to verify the full integration in the app.
